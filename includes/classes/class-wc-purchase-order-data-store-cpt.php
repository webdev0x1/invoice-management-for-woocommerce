 ...
 * @version  3.0.0
 */
class WC_Purchase_Order_Data_Store_CPT extends Abstract_WC_Order_Data_Store_CPT implements WC_Object_Data_Store_Interface, WC_Order_Data_Store_Interface {
	...
	public function get_order_count( $status ) {
		global $wpdb;
		return absint( $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( * ) FROM {$wpdb->posts} WHERE post_type = 'purchase_order' AND post_status = %s", $status ) ) );
	}
	...
	public function get_orders( $args = array() ) {
		wc_deprecated_function( 'WC_Purchase_Order_Data_Store_CPT::get_orders', '3.1.0', 'Use wc_get_orders instead.' );
	...
	public function search_orders( $term ) {
		...
		$search_fields = array_map(
			'wc_clean',
			apply_filters(
				'woocommerce_purchase_order_search_fields',
				array(
					'_billing_address_index',
					'_shipping_address_index',
					'_billing_last_name',
		...
		return apply_filters( 'woocommerce_purchase_order_search_results', $order_ids, $term, $search_fields );
	}
	...
	private function prime_refund_caches_for_order( $order_ids, $query_vars ) {
		if ( ! isset( $query_vars['type'] ) || ! ( 'purchase_order' === $query_vars['type'] ) ) {
			return;
		}
