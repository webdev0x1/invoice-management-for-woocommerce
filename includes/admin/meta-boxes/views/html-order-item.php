...
<tr class="item <?php echo esc_attr( $row_class ); ?>" data-order_item_id="<?php echo esc_attr( $item_id ); ?>">
	...
	<td class="name" data-sort-value="<?php echo esc_attr( $item->get_name() ); ?>">
		...
		?>
		<input type="hidden" class="order_item_id testing" name="order_item_id[]" value="<?php echo esc_attr( $item_id ); ?>" />
	...
	$val = get_post_meta($product->get_id(), 'inv_empye_supplier', true);
            ...
            echo '<td colspan="1">' . $product->get_weight() . '<td>';
           ...
           // echo '<td colspan="2"><input type="text" class="" name="'.$product->get_id().'_purpose" maxlength="10" /></td>';
           // echo '<td colspan="2"><input type="text" class="date-picker" name="'.$product->get_id().'_date_of_purchase" maxlength="10" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" id="dp1621912214881"/></td>';
	?>
	<td colspan="2">
