...
class INV_WC_AJAX {
	...
	public static function init() {
		add_action( 'init', array( __CLASS__, 'define_ajax_custom' ), 0 );
		add_action( 'template_redirect', array( __CLASS__, 'do_wc_ajax_custom' ), 0 );
	 ...
	 */
	public static function define_ajax_custom() {
		// phpcs:disable
	 ...
	 */
	public static function do_wc_ajax_custom() {
		global $wp_query;
	...
	public static function add_order_item() {
		...
		//parse_str($_POST['data'], $array);
			...
			//	print_r($ar);
			//	if( isset( $ar['purpose'] ) ) {
                    //$item->update_meta_data( '_purpose', sanitize_text_field( $ar['purpose'] ) );
             //       wc_update_order_item_meta('_purpose', sanitize_text_field( $array['purpose'] ));
                   // wc_add_order_item_meta('_purpose', sanitize_text_field( $ar['purpose'] ));
                    
                    //$item->save();
           //     }

           //     if( isset( $ar['date_of_purchase'] ) ) {
                    //$item->update_meta_data( '_date_of_purchase', sanitize_text_field( $ar['date_of_purchase'] ) );
                   // wc_add_order_item_meta('_date_of_purchase', sanitize_text_field( $ar['date_of_purchase'] ));
           //         wc_update_order_item_meta('_date_of_purchase', sanitize_text_field( $ar['date_of_purchase'] ));
                    //$item->save();
           //     }
          //  }
            //$order->save();
	...
	private static function maybe_add_order_item( $order_id, $items, $items_to_add ) {
		try {
			...
			foreach ( $items_to_add as $item ) {
				...
				$cost = get_post_meta($product_id, 'inv_empye_supplier_price', true);
					//wc_update_order_item_meta($item_id,'cost',$cost);
					//$item->update_meta_data( 'cost', $cost );
					//$item->save();
					//$item->set_total( $cost * $qty );
					//$item->set_subtotal( $cost * $qty); 
					//$item->save();

				$item_id                 = $order->add_product( $product, $qty, ['subtotal' => $cost, 'total' => ($cost * $qty)]);
				$item_old 				 = $item;
				...
				// We do not perform any stock operations here because they will be handled when order is moved to a status where stock operations are applied (like processing, completed etc).
					...
					if( isset( $item_old['purpose'] ) ) {
						$item->update_meta_data( '_purpose', sanitize_text_field( $item_old['purpose'] ) );
						//wc_update_order_item_meta('_purpose', sanitize_text_field( $array['purpose_'.$item_id] ));
						//wc_add_order_item_meta('_purpose', sanitize_text_field( $array['purpose_'.$item_id] ));
						
						$item->save();
					}
	
					if( isset( $item_old['date_of_purchase'] ) ) {
						$item->update_meta_data( '_date_of_purchase', sanitize_text_field( $item_old['date_of_purchase'] ) );
						//wc_add_order_item_meta('_date_of_purchase', sanitize_text_field( $array['date_of_purchase_'.$item_id] ));
						//wc_update_order_item_meta('_date_of_purchase', sanitize_text_field( $array['date_of_purchase_'.$item_id] ));
						$item->save();
					}
					//echo ;die();
					$val = get_post_meta($item['product_id'], 'inv_empye_supplier', true);
					$address =  get_post_meta($val, 'inv_empye_direct_name', true)."\n";
					$address .=  get_post_meta($val, 'inv_empye_street', true)."\n";
					...
					$address .=  get_post_meta($val, 'inv_empye_zipcode', true)."\n";
					$address .=  get_post_meta($val, 'inv_empye_direct_email', true)."\n";
					$address .=  get_post_meta($val, 'inv_empye_direct_phone', true)."\n";
					
                if( null !==  get_the_title($val[0]) ) {
				...
				if( isset( $item_old['supplier_price'] ) ) {
					$item->update_meta_data( '_supplier_price', sanitize_text_field( $item_old['supplier_price'] ) );
					$item->save();
				}
	...
	public static function save_order_items() {
		...
		if ( isset( $_POST['order_id'], $_POST['items'] ) ) {
			...
			foreach ( $order->get_items() as $item_id => $item ) {
                ...
                if( isset( $array['purpose_'.$item_id] ) ) {
                    $item->update_meta_data( '_purpose', sanitize_text_field( $array['purpose_'.$item_id] ) );
                    //wc_update_order_item_meta('_purpose', sanitize_text_field( $array['purpose_'.$item_id] ));
                    //wc_add_order_item_meta('_purpose', sanitize_text_field( $array['purpose_'.$item_id] ));
                ...
                if( isset( $array['date_of_purchase_'.$item_id] ) ) {
                    $item->update_meta_data( '_date_of_purchase', sanitize_text_field( $array['date_of_purchase_'.$item_id] ) );
                    //wc_add_order_item_meta('_date_of_purchase', sanitize_text_field( $array['date_of_purchase_'.$item_id] ));
                    //wc_update_order_item_meta('_date_of_purchase', sanitize_text_field( $array['date_of_purchase_'.$item_id] ));
                    $item->save();
                }

				$val = get_post_meta($item['product_id'], 'inv_empye_supplier');
					$address =  get_post_meta($val[0], 'inv_empye_direct_name', true)."\n";
					$address .=  get_post_meta($val[0], 'inv_empye_street', true)."\n";
					$address .=  get_post_meta($val[0], 'inv_empye_city', true).", ";
					$address .=  get_post_meta($val[0], 'inv_empye_state', true).", ";
					$address .=  get_post_meta($val[0], 'inv_empye_country', true).", ";
					$address .=  get_post_meta($val[0], 'inv_empye_zipcode', true)."\n";
					$address .=  get_post_meta($val[0], 'inv_empye_direct_email', true)."\n";
					$address .=  get_post_meta($val[0], 'inv_empye_direct_phone', true)."\n";
					
                if( null !==  get_the_title($val) ) {
					$item->update_meta_data( '_supplier_name', sanitize_text_field( get_the_title($val[0]) ) );
                    $item->save();
                }
				...
				if( isset( $array['supplier_price_'.$item_id] ) ) {
                    $item->update_meta_data( '_supplier_price', sanitize_text_field( $array['supplier_price_'.$item_id] ) );
                    //wc_add_order_item_meta('_date_of_purchase', sanitize_text_field( $array['date_of_purchase_'.$item_id] ));
                    //wc_update_order_item_meta('_date_of_purchase', sanitize_text_field( $array['date_of_purchase_'.$item_id] ));
                    $item->save();
                }
