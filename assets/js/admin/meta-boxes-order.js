...
jQuery( function ( $ ) {
	...
	var wc_meta_boxes_order_items = {
		...
		edit_item: function() {
			...
			$( this ).closest( 'tr' ).find( '.form-field input' ).removeAttr('readonly');
			$( this ).closest( 'tr' ).find( '.form-field input.date' ).addClass('date-picker');
			$( '.date-picker-field, .date-picker' ).datepicker({
				dateFormat: 'yy-mm-dd',
				numberOfMonths: 1,
				showButtonPanel: true
				...
				if ( 'wc-modal-add-products' === target ) {
					...
					$( rows ).each( function() {
						var item_id = $( this ).find( ':input[name="item_id"]' ).val(),
							purpose = $( this ).find( ':input[name="purpose"]' ).val(),
							date_of_purchase = $( this ).find( ':input[name="date_of_purchase"]' ).val(),
